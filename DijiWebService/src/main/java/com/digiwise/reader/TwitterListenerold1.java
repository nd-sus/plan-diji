...
public class TwitterListenerold1 {
	// facebook handler
	static Logger logger = Logger.getLogger(TwitterListenerold1.class);
	byte[] body;
	String routingKey;
	...
	public TwitterListenerold1(byte[] body, String routingKey,
			ExecutorService executor,
		...
		try {
			...
			try {
				...
				for (Status i : data) {
					...
					logger.info("start processing " + i.getId() + " is "
							+ (System.currentTimeMillis()));
					/*
					 * logger.info("###############Start#############" +
					 * i.getId());
					...
					try {
						...
						if (i.getMediaEntities() != null
							...
							int j = 0;
							for (twitter4j.MediaEntity c : i.getMediaEntities()) {
								TwitterMediaEntities entity = new TwitterMediaEntities();
			...
			logger.info("start of the twitter Thread: " + " at :"
					+ (System.currentTimeMillis()));
		} catch (Exception e) {
			logger.error("Exception:" + e.getMessage());
			e.printStackTrace();
