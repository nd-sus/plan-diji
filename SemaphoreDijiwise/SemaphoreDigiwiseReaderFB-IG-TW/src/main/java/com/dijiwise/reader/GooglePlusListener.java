...
public class GooglePlusListener {
	...
	public static Future<?> getUserGoogleFeeds(final byte[] body,
			final String routingKey, ExecutorService executor, 
		...
		t = new Thread() {
			...
			@Override
			public void run() {
				this.setName("Dijiwise-Thread-GooglePlus-Listener"
						+ counterValue);
				logger.info("start of the facebook Thread: " + this.getName()
				...
				try {
					...
					logger.info("start of the fb Thread: " + this.getName()
							+ " at :" + (System.currentTimeMillis()));
				} catch (Exception e) {
